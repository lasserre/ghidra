...
bool ASTBuilder::pushEquate(uintb val,int4 sz,const EquateSymbol *sym, const Varnode *vn,const PcodeOp *op)
{
    ...
    if (modval == val) {
        pushSymbolAST(const_cast<EquateSymbol*>(sym), op ? op->getAddr().getOffset() : 0);
        return true;
    }
    modval = (~baseval) & mask;
...
void ASTBuilder::push_integer(Datatype* dt, uintb val,int4 sz,bool sign, const Varnode *vn,const PcodeOp *op)
{
    ...
    if (print_negsign) {
        ...
        */
        ...
        IntegerLiteral* lit = new IntegerLiteral(int_dt, val, op ? op->getAddr().getOffset() : 0);
        unop->addChild(lit);
        return;
    }
...
void ASTBuilder::push_bool(uintb value, int size, const PcodeOp* op)
{
    ...
    currentASTNode()->addChild(new IntegerLiteral(
            ...
            value, op ? op->getAddr().getOffset() : 0));
}
...
void ASTBuilder::pushUnnamedLocation(const Address &addr, const Varnode *vn,const PcodeOp *op)
{
    ...
    DeclRefExpr* refexpr = new DeclRefExpr(vdecl, VAR_DECL, op ? op->getAddr().getOffset() : 0);
    currentASTNode()->addChild(refexpr);
            ...
            const Varnode *vn,const PcodeOp *op,int4 inslot)
{
    ...
    pushSymbolAST((Symbol*)sym, op ? op->getAddr().getOffset() : 0);
    ...
    if (!sym) {
        ...
        if (symboloff == -1) {
            ...
            if (!sym->getType()->needsResolution()) {
                pushSymbolAST(sym, node->op ? node->op->getAddr().getOffset() : 0);
                return;
            }
            symboloff = 0;
        }
...
void ASTBuilder::opBranchind(const PcodeOp *op)
{
    ...
    if (isSet(comma_separate)) {
        BinaryOperator* comma_op = insertCommaOperator(ss, op ? op->getAddr().getOffset() : 0);
